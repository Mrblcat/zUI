zUI:RegisterComponent("zUnitframes", function()
	...
	--local C = zUI_config
	--mrbcat -20230720 自定义函数获取双倍经验值
	local function GetXpEX()
		local p = "player"
		local  m, r, t =  UnitXPMax(p), GetXPExhaustion(), "";
		if -1 == (r or -1) then
			t = " 双倍: 0/30"
		else
			t = " 双倍: " .. (math.floor(20 * r / m + 0.5)) .. "/30"
		end
		return t
	end
			...
			if (valueMax > 0) then
				...
				if (value == 0 and textStatusBar.zeroText) then
					...
					if (C.unitframes.percentages == "1") then
						--MRBCAT 添加双倍经验显示
						if textStatusBar:GetName() == "MainMenuExpBar" then
							--string:SetText(true_format(v)..'/'..true_format(max).. ' \226\128\148 ' ..percent..'%')
							string:SetText("当前：" .. percent .. '%' .. expt)
						else
						...
						if textStatusBar:GetName() == "MainMenuExpBar" then
							--string:SetText(true_format(v)..'/'..true_format(max).. ' \226\128\148 ' ..percent..'%')
							string:SetText("当前：" .. true_format(v) .. '/' .. true_format(max) .. expt)
						else
	...
	function Initial_UnitframesLayout()
		...
		if C.unitframes.Hide_Player_name == "1" then
			PlayerName:Hide()
		end
		if (C.unitframes.compactmode == "1") then
		...
		if (C.unitframes.nameoutline == "1") then
			PlayerName:SetFont(STANDARD_TEXT_FONT, C.unitframes.namefontsize, "OUTLINE");
			TargetName:SetFont(STANDARD_TEXT_FONT, C.unitframes.namefontsize, "OUTLINE");
			TargetofTargetName:SetFont(STANDARD_TEXT_FONT, C.unitframes.namefontsize - 1, 'OUTLINE');
			PetName:SetFont(STANDARD_TEXT_FONT ,C.unitframes.namefontsize - 1, "OUTLINE");
			...
			PlayerName:SetFont(STANDARD_TEXT_FONT, C.unitframes.namefontsize);
			TargetName:SetFont(STANDARD_TEXT_FONT, C.unitframes.namefontsize);
			TargetofTargetName:SetFont(STANDARD_TEXT_FONT, C.unitframes.namefontsize - 1);
			PetName:SetFont(STANDARD_TEXT_FONT, C.unitframes.namefontsize - 1);
	...
	function zUI.MobHealth:PowerUpdate()
		...
		if max == 0 or cur == 0 or percent == 0 then
			string:SetText()
			return
		end
	...
	f:SetScript('OnEvent', function()
		if arg1 == 'STATUS_BAR_TEXT' or event == 'PLAYER_ENTERING_WORLD' then
			...
			if GetCVar 'statusBarText' == '0' then
				zTargetHealthText:Hide()
				zTargetPowerText:Hide()
				...
				zTargetHealthText:Show()
				zTargetPowerText:Show()
				--Initialize();
			end
