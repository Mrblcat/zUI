...
--稳固射击
libcast.customcast[strlower(steadyshot)] = function(begin)
  if begin then
    local duration = 1500
...
hooksecurefunc("UseContainerItem", function(bag, slot)
  zUI.ICON = GetContainerItemInfo(bag, slot)
end)
hooksecurefunc("CastSpell", function(id, bookType)
  if GetSpellCooldown(id, bookType) ~= 0 then
    zUI.ICON = GetSpellTexture(id, bookType)
...
hooksecurefunc("CastSpellByName", function(spellName, target)
  local _, _, spellrank = string.find(spellName, "(%d+)")
  
	if spellrank then
		Icon = zUI.api.select(3, zUI.api.libspell.GetSpellInfo(spellName))
	else
		Icon = zUI.api.select(3, zUI.api.libspell.GetSpellInfo(spellName))
	end
  zUI.ICON = Icon
...
hooksecurefunc("UseAction", function(slot, target, button)
  ...
  zUI.ICON = GetActionTexture(slot)
  --zPrint(spellName)
  CastCustom(spellName)
end, true)
